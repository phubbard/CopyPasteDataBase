...
/// <c>created_at</c>. Mirrors the macOS <c>HistoryExporter</c>
/// contract byte-for-byte: CSV is RFC-4180 with exactly the 12
/// columns below in order; HTML is self-contained with a dark-mode
/// <c>@media</c> block and no external assets; Markdown is a
/// paragraph per entry. Timestamps are ISO-8601.
/// </summary>
public static class HistoryExporter
{
    ...
    public static string RenderMarkdown(IReadOnlyList<Row> rows)
    {
        ...
        {
            var pin = r.Pinned ? "📌 " : "";
            o.Append($"## {pin}{r.Headline}\n\n");
            var meta = new List<string> { $"**{r.Kind}**" };
            if (!string.IsNullOrEmpty(r.AppName))    meta.Add(r.AppName!);
            if (!string.IsNullOrEmpty(r.DeviceName)) meta.Add(r.DeviceName!);
            meta.Add(Ts(r.CreatedAt));
            if (r.Evicted) meta.Add("_(body evicted)_");
            o.Append(string.Join(" · ", meta) + "\n\n");
            if (!string.IsNullOrEmpty(r.TextPreview) && r.TextPreview != r.Headline)
            {
                o.Append($"```\n{r.TextPreview}\n```\n\n");
            }
            if (!string.IsNullOrEmpty(r.OcrText))
            {
                o.Append($"> OCR: {Prefix(r.OcrText!, 500)}\n\n");
            }
            if (!string.IsNullOrEmpty(r.ImageTags))
            {
                o.Append($"> Tags: {r.ImageTags}\n\n");
            }
            o.Append("---\n\n");
        }
        return o.ToString();
    ...
    public static string RenderCsv(IReadOnlyList<Row> rows)
    {
        ...
        var o = new StringBuilder();
        o.Append("id,kind,pinned,evicted,created_at,captured_at,source_app,device,headline,text_preview,ocr_text,image_tags\n");
        foreach (var r in rows)
        {
            var cols = new[]
            {
    ...
    public static string RenderHtml(IReadOnlyList<Row> rows)
    {
        ...
        foreach (var r in rows)
        {
            ...
            if (!string.IsNullOrEmpty(r.OcrText))
            {
                o.Append($"  <div class=\"badges\">OCR: {Esc(Prefix(r.OcrText!, 500))}</div>\n");
            }
            o.Append("</div>\n");
        }
        o.Append("</body></html>\n");
