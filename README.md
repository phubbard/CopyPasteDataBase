...
| **Canonical-hash v2** | Semantic content identity (image → file → url → normalized text → fallback) replacing full-flavor-set SHA-256. Design + evidence in [`docs/canonical-hash-v2.md`](docs/canonical-hash-v2.md) and [`docs/canonical-hash-v2-evidence.md`](docs/canonical-hash-v2-evidence.md). Prerequisite for iOS read/write peer. New CloudKit zone `cpdb-v3`; chunked resumable local rehash. **Steps 0–3 done**: frozen reference impl + cross-platform vectors ([`Tools/gen_hash_vectors.py`](Tools/gen_hash_vectors.py), [`Tests/Fixtures/hash-vectors-v2.json`](Tests/Fixtures/hash-vectors-v2.json), 26 vectors); simulation gate passed (475 merge-clusters, **0 false merges**); Swift [`ContentIdentity`](Sources/CpdbShared/Capture/ContentIdentity.swift); capture path on v2; resumable local migration ([`IdentityCutover`](Sources/CpdbShared/Store/IdentityCutover.swift)) — chunked rehash + collision merge + reseed, the `cpdb-v3.db` skew fence, `cpdb storage --verify-hashes`, adversarial-reviewed (6 findings fixed), verified on the real production snapshot with **0 hash mismatches**. ⚠️ main is mid-cutover — do not install until R1 assembles. **R1 shipped + run on the primary Mac** (2.11.0): full Steps 0–4 — frozen contract, ContentIdentity, v2 capture, resumable migration, §5.3 pull-side conflict resolution, launch wiring + `cpdb migrate-identity` CLI + cross-process lock + backup script. Primary cutover: 9,856 rehashed / 479 clusters merged / **0 hash mismatches**, now on the `cpdb-v3` zone. Secondary devices (axiom/thor/iPhone) safely partitioned on cpdb-v2 pending the §5.4 choreography | ✅ primary / ⏳ fleet |
