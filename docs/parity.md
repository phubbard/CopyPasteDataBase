...
| Auto-update | ✅ Sparkle 2 (v2.9.1) | — | ✅ v1.11.0 (x64) | Mac: EdDSA-signed appcast at `releases/latest/download/appcast.xml`, daily background check + "Check for Updates…" menu item, prompt-not-silent. Windows: `UpdateService` over Velopack `GithubSource`, check 30s after launch + every 24h + on-demand tray item, download-then-prompt-then-restart (never silent), skips dev/portable builds. `release-installer.ps1` publishes the `releases.win.json` + `.nupkg` feed. **x64 only** — Velopack 0.0.1298 emits an identically-named `win` channel manifest + nupkg for both arches so a single GitHub release can't carry both; arm64 updates by re-download. Per-arch channels tracked as a follow-up |
