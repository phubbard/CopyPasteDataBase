...
| Popup summon perf (instrumented; lazy strip + async thumbnails + deferred status queries) | ✅ 3.2.2 (868ms → 113ms cold / 36ms warm on M2, 10k entries; permanent `popup-perf` log line) | — (List is natively lazy; no popup) | ⏳ | Method + WinUI checklist: [`docs/handoffs/windows-popup-perf.md`](handoffs/windows-popup-perf.md). Windows: verify strip virtualization, kill per-item connection opens, defer aggregates past first paint, async off-thread bitmap decode. |
