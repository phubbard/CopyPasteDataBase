...
| **Canonical-hash v2** | Semantic content identity (image → file → url → normalized text → fallback) replacing full-flavor-set SHA-256. Design + evidence in [`docs/canonical-hash-v2.md`](docs/canonical-hash-v2.md) and [`docs/canonical-hash-v2-evidence.md`](docs/canonical-hash-v2-evidence.md). Prerequisite for iOS read/write peer. New CloudKit zone `cpdb-v3`; chunked resumable local rehash. **Steps 0–2 done**: frozen reference impl + cross-platform vectors ([`Tools/gen_hash_vectors.py`](Tools/gen_hash_vectors.py), [`Tests/Fixtures/hash-vectors-v2.json`](Tests/Fixtures/hash-vectors-v2.json), 26 vectors); simulation gate passed (475 merge-clusters, **0 false merges**); Swift [`ContentIdentity`](Sources/CpdbShared/Capture/ContentIdentity.swift) green against all vectors; capture path on v2 (stamping, union bump, ingest-level transient guard, log-only window). ⚠️ main is mid-cutover — do not install until R1 assembles. Next: the local migration (Step 3) | ⏳ capture-wired |
