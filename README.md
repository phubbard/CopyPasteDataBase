...
| 2.9+ | Browse window (full-screen, tiled, scrolling grid) · App Store submission for the iOS companion · size-budget eviction | ⏳ |
| **Legacy zone GC** | `cpdb sync gc-zone` (dry-run by default, `--force` to delete, refuses the live zone) + an entitled-app execution hook (`defaults write net.phfactor.cpdb gcZoneRequest …` — the CLI process can't hold iCloud entitlements). The abandoned `cpdb-v2` CloudKit zone (~9k pre-hash-v2 records) deleted + verified 2026-07-13; every device confirmed on `cpdb-v3` first | ✅ |
...
| **3.2.1 — OCR self-heal + legacy-zone GC** | Periodic jittered image-analysis sweep (per-entry error containment, giant-image downscale guard: byte + pixel-dimension thresholds), analysis results + `analyzed_at` sentinel now sync via push-after-analyze, CLI iCloud-entitlement preflight (exit 2, no more CKContainer SIGTRAP), `cpdb sync gc-zone` + entitled-app hook — legacy `cpdb-v2` zone deleted + verified | ✅ |
| **3.2.2–3.2.5 — popup summon perf** | Now regression-tested: a rendered-layout test measures the actual card height in a real PopupRootView render (would have caught the v3.2.3 clipping at 15.5pt off) plus a panel-height geometry contract. Instrument-first tuning of hotkey→first-frame: 868ms → 113ms cold / 36ms warm (M2, 10k entries). Lazy card strip, no forced hidden-window draw, status queries deferred past first paint, per-card DB opens (142/summon) eliminated via environment store + async off-main thumbnail decode. Permanent `popup-perf` log line for regression-catching. Windows briefed: [`docs/handoffs/windows-popup-perf.md`](docs/handoffs/windows-popup-perf.md) | ✅ |
| **Apple API roadmap** | Planning survey of current OS capabilities (verified against Apple docs, Aug 2026): semantic search via on-device embeddings, table→CSV document OCR, QR ingest, Foundation Models enrichment, App Intents/Spotlight surface, CKSyncEngine, and the two platform threats (pasteboard-privacy enforcement, macOS 27 container-access denial hitting the Paste.db importer). See [`docs/apple-api-roadmap.md`](docs/apple-api-roadmap.md) | 🗺 planning |
| **3.3.0 — semantic search + AI enrichment** | On-device semantic search (NLContextualEmbedding + FTS5 hybrid, synced vectors), action chips (dates/addresses/tracking/QR), Copy-as-Table from screenshots (Vision document OCR, macOS 26), Foundation Models auto-titles/summaries (26+, gated), Shortcuts/Spotlight intents + deep links, pasteboard-privacy preparedness (accessBehavior monitor, alert-free pre-read, secure-input skip). Six parallel-built streams, adversarially reviewed; schema v12–v14 | ✅ |
| **3.3.1 — paste reliability + sync throttle** | Double-click paste made chip-proof (`highPriorityGesture` + paste-by-clicked-id, persistent `paste:` logging — a silent paste failure can no longer leave zero forensic trace), CloudKit push/pull throttle gate honoring server Retry-After + poisoned-record queue rotation + multi-batch drain (fixes a 25 h / 6,600-row push starvation), CI repaired (macos-26 runners, live-ML test gates, 30-min timeout). Chip double-click paste verified live against Claude Desktop via the new persistent logs. Shipped 2026-08-29 | ✅ |
