...
| Generic HTML title scrape (og:title → twitter:title → `<title>`) | ✅ v2.7.0 | — | ✅ v1.3.0 / WP-aware v1.30.0 | regex-based, see `windows/CpdbWin.Core/Analysis/LinkMetadataParser.cs`. **Windows v1.30.0 deviation:** WordPress sites (detected via `<meta name="generator" content="WordPress…">`) reverse the order to `<title>` → og:title → twitter:title, because WP themes consistently put the rich "Title – Tagline" form in `<title>` while `og:title` is the bare post slug. WP backs ~40-60% of public sites, so the deviation has high coverage; non-WP pages keep the default order. macOS could adopt the same; not yet ported |
