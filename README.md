...
| **Undo (delete + pin)** | Reversible delete + pin on Mac (⌘Z/⌘⇧Z) and iOS (undo snackbar + shake). Multi-level undo/redo stack. Cross-device-correct via a `modified_at` last-writer-wins column (v11) — an undone delete propagates instead of staying deleted on a sibling. Foundation (migration + LWW sync + `restore`) landed | ⏳ in progress |
