...
| **iOS read/write** | Promote the iOS companion from read-only to a clipboard writer (text/links first). Capture behind a default-off toggle; `detectPatterns`-gated reads (no spurious "pasted from" banners); routes through the shared `Ingestor`. First pass on branch `claude/ios-readwrite` — macOS-verified, iOS-only code not yet compiled on device | ⏳ in progress |
| **Canonical-hash v2** | Semantic content identity (image → file → url → normalized text → fallback) replacing full-flavor-set SHA-256. Design + evidence in [`docs/canonical-hash-v2.md`](docs/canonical-hash-v2.md) and [`docs/canonical-hash-v2-evidence.md`](docs/canonical-hash-v2-evidence.md). Prerequisite for iOS read/write peer. New CloudKit zone `cpdb-v3`; chunked resumable local rehash | ⏳ design-frozen |
