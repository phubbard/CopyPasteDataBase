...
| **Relay sync substrate** | Cross-platform replacement for CloudKit. Phase A done: blind-PSK protocol spec ([`docs/relay-protocol.md`](docs/relay-protocol.md)) + Worker scaffold ([`cpdb-relay/`](cpdb-relay/)). Deep design review complete ([`docs/relay-deep-analysis.md`](docs/relay-deep-analysis.md)): Cloudflare confirmed at $0/mo, hybrid topology (CloudKit stays for Apple, relay bridges Windows), but v1 spec found unimplementable as written — a v2 protocol revision (meta/body split, seq change-log, IETF ChaCha20-Poly1305, reseed-as-migration) is the next step, gated on Windows hash-v2. Accounts/tiers/OAuth design parked in [`docs/relay-v2-accounts-roadmap.md`](docs/relay-v2-accounts-roadmap.md) — deferred, clients not started | ⏳ |
