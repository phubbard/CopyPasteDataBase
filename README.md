...
| **Undo (delete + pin)** | Reversible delete + pin on Mac (⌘Z/⌘⇧Z, with a transient hint) and iOS (undo snackbar + shake). Multi-level [`UndoCoordinator`](Sources/CpdbShared/UndoCoordinator.swift) stack. Cross-device-correct via a `modified_at` last-writer-wins column (v11) — an undone delete propagates instead of staying deleted on a sibling. Foundation + Mac UX done; iOS next | ⏳ in progress |
