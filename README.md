...
| **2.8.0** | Link metadata enrichment, finished — preview thumbnails (og:image / twitter:image / Wikipedia REST API / favicon-fallback) · live popup updates while backfill runs · capture-wake immediate enrichment · hover tooltips with source app + device + timestamp · URL-shaped text reclassifies to `kind=link` · `loginwindow` ignore + cross-time link dedupe · "Retry empties" + transient-error handling · single-instance guard · popup close button | ✅ |
