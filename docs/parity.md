...
| Match-source badges | ✅ v1.2 | — | ✅ v1.47.0 | Small pill on each search-result row indicating which FTS5 column produced the hit. `EntryRepository.Search` splices `highlight(entries_fts, N, char(1), char(2))` columns into the SELECT list (text=1, ocr_text=3, image_tags=4, matching `Store/Schema.cs`); `ClassifyMatchSource` maps sentinel presence to `MatchSource.{Text,Ocr,Tag,Multiple}` (Mac's names preserved so `FtsIndexTests` port unchanged). `EntryViewModel` computes label + tooltip + per-source background brush (accent/green/purple), and `MainWindow.xaml`'s row template renders the pill in col 2 alongside the OCR-availability badge. Plain-text hits show no badge — badging the modal case is noise. **Mac has a `.semantic` case for hybrid-search re-rank hits; not ported yet** because Windows' `HybridRank.Fuse` drops per-hit source when hydrating via `RowsByIds`. |
